    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.infoSection}>
          <InfoItem 
            label="Họ và tên"
            value={authState.user?.name || 'Tùy Phong'}
          />
          <InfoItem 
            label="Số điện thoại"
            value={authState.user?.phone || '0384 123 456'}
          />
          <InfoItem 
            label="Email"
            value={authState.user?.email || 'tuyphong@slm.vn'}
          />
          <InfoItem 
            label="Ngày sinh"
            value={authState.user?.birthDate || '14/01/1993'}
          />
          <InfoItem 
            label="Giới tính"
            value={authState.user?.gender || 'Nam'}
          />
          <InfoItem 
            label="Số CCCD/Hộ chiếu"
            value={authState.user?.idNumber || '1234 5678 9000'}
          />
          <InfoItem 
            label="Ngày cấp"
            value={authState.user?.issueDate || '01/01/2020'}
          />
          <InfoItem 
            label="Nơi cấp"
            value={authState.user?.issuePlace || 'Cục Cảnh sát'}
          />
          <InfoItem 
            label="Địa chỉ liên hệ"
            value={authState.user?.address || 'Somewhere over the rainbow'}
          />
        </View>
